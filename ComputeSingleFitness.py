...
def get_fitness(R,pizza_matrix, alpha, beta, gamma,mu, L,H):

    overlap_matrix = np.zeros(pizza_matrix.shape)
    
    fitness = 0
    for r_i,r_e,c_i,c_e in R:
        slice_matrix = pizza_matrix[r_i:r_e+1,c_i:c_e+1];
        ...
        slice_big_matrix = np.zeros(pizza_matrix.shape)
